...
log "INFO" "Control $CONTROL_ID: Ensure a Firewall is Created (tag=$ENV_TAG)"
...
# Firewalls in project
firewalls_json=$(run_doctl_json compute firewall list)

# Any firewall applying by tag to this env?
...
if [[ "$pass" != "true" ]]; then
  echo "FAIL [$CONTROL_ID] Some droplets are not protected by a firewall"
  exit 1
fi

echo "PASS [$CONTROL_ID] All droplets are protected by a firewall"
