...
log "INFO" "Control $CONTROL_ID: Ensure auditd is enabled (tag=$ENV_TAG)"
...
for id in "${droplet_ids[@]}"; do
  ...
  log "INFO" "Checking auditd on $name ($ip)"

  if ! pkg_installed=$(run_remote "$ip" "dpkg -s auditd >/dev/null 2>&1 && echo yes || echo no"); then
  ...
  enabled=$(run_remote "$ip" "systemctl is-enabled auditd >/dev/null 2>&1 && echo yes || echo no" || echo no)
  active=$(run_remote "$ip" "systemctl is-active auditd >/dev/null 2>&1 && echo yes || echo no" || echo no)

  reasons=()
  [[ "$pkg_installed" == "yes" ]] || reasons+=("auditd package not installed")
  [[ "$enabled" == "yes" ]] || reasons+=("auditd service not enabled")
  [[ "$active" == "yes" ]] || reasons+=("auditd service not running")
  ...
  if [[ ${#reasons[@]} -gt 0 ]]; then
    ...
    failed_entries+=("$(
      jq -n \
        ...
        --arg enabled "$enabled" \
        --arg active "$active" \
        '{droplet:$droplet,ip:$ip,reason:$reason,checks:{pkg_installed:$pkg_installed,service_enabled:$enabled,service_active:$active}}'
    )")
  else
    log "INFO" "$name: auditd enabled and running"
  fi
done
...
if [[ "$pass" != "true" ]]; then
  echo "FAIL [$CONTROL_ID] auditd not enabled/running on some droplets"
  exit 1
fi

echo "PASS [$CONTROL_ID] auditd enabled and running on all droplets"
