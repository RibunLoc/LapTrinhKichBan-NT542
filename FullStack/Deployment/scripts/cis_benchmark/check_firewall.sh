...
ADMIN_CIDRS_ENV="${ADMIN_CIDRS:-27.65.249.253/32}"
...
for fw_id in "${fw_ids[@]}"; do
  ...
  # 1) Không cho inbound TCP port lạ ngoài 22/80/443
  invalid_ports=$(echo "$fw" | jq '
    [
      .inbound_rules[]
      | select(
          .protocol == "tcp"
          and (.ports | test("^(22|80|443)$") | not)
        )
    ] | length
  ')

  if [[ "$invalid_ports" -gt 0 ]]; then
    offending_ports=$(echo "$fw" | jq -r '
      .inbound_rules[]
      | select(
          .protocol == "tcp"
          and (.ports | test("^(22|80|443)$") | not)
        )
      | "- protocol=\(.protocol), ports=\(.ports), sources=\(.sources.addresses // [])"
    ')
    log "ERROR" "Firewall $name allows TCP ports khác 22/80/443:"
    printf '%s\n' "$offending_ports" >&2
    ...
    failed_entries+=("$(
      jq -n \
        --arg control "2.1.2" \
        --arg fw "$name" \
        --arg reason "Inbound TCP port outside 22/80/443" \
        '{control:$control,firewall:$fw,reason:$reason}'
    )")
  fi

  # 2) SSH không mở 0.0.0.0/0
  open_ssh_world=$(echo "$fw" | jq '
    [
      .inbound_rules[]
      | select(.protocol=="tcp" and .ports=="22")
      | .sources.addresses[]?
      | select(.=="0.0.0.0/0" or .=="::/0")
    ] | length
  ')
  ...
  # Tất cả SSH sources phải nằm trong ADMIN_CIDRS
  if [[ $(jq -n --argjson allow "$allowed_list_json" --argjson actual "$ssh_sources" '
            ([ ($actual[]? as $a | select(($allow|index($a))!=null)) ] | length) == ($actual|length)
          ') != "true" ]]; then
    ...
    failed_entries+=("$(
      jq -n \
        --arg control "2.1.3" \
        --arg fw "$name" \
        --arg reason "SSH sources not in ADMIN_CIDRS" \
        '{control:$control,firewall:$fw,reason:$reason}'
    )")
  fi
done
